...
*The Debounce technique allows to "group" multiple sequential calls in a single one.*
...
The idea is simple, if a page show a static map, we can show it again on another page, without having to pay more.
...
*This is not a practical tip, but it allows to understand how browser cache works and eventually trying to simulate it.*
...
90% of the time it is preferable to use the `sessions` because they allow to `group requests` by paying a single fee. There are cases where you need to make a few requests, we could take **advantage** of the non-session method ( **per request** ) and pay less.
...
Google correct any user typo, using `PlacesService` or `Autocomplete`
> Ro**n**e, Italy ==> Ro**m**e, Italy // place_id: ChIJN1t_tDeuEmsRUsoyG83frY4

Once the user has specified the **address**, to retrive more information and **details** about that location, an additional call must be made, specifying the `place_id` of the searched address.
...
Other useful tips are to **monitor** usage of your API to catch anomalies, set a **budget alert**, **restrict** your API keys to specific IP, using a **native app** to view maps (it's Free!) instead of a web application...
...
This makes us reflect on the monopolisation of a service that can make us slaves, what would happen if Google made Google Analytics or Tag Manager paid?, as it did 2 years ago for Google Maps?
